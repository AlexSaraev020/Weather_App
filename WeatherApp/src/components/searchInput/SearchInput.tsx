...
export const SearchInput:React.FC<SearchInputProps> = ({setSearchedCity,handleSubmit}) => {
    return (
        <form onSubmit={handleSubmit} className="relative w-3/12 mx-auto">
            <input required onChange={(e)=>setSearchedCity(e.target.value)} className="w-full py-2 px-4 border-2 bg-slate-800/50 backdrop-blur-sm border-slate-500 rounded-md shadow-sm focus:outline-none" type="search" placeholder="Search" />
            <button type="submit" className="absolute inset-y-0 right-0 flex items-center px-4 text-slate-300 bg-slate-800/50 border-t-2 border-b-2 border-r-2 border-slate-500 rounded-r-md focus:outline-none ">
