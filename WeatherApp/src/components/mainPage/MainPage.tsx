    ...
    return (
        <div className="w-full h-full flex items-center relative justify-center">
            ...
            <div className="bg-black/40 space-y-14 text-white w-full h-full z-10 flex flex-col items-center justify-center p-4">
                {weather && (
                    <div className={`pb-20 sm:pb-0 sm:pl-24 relative w-full sm:w-7/12 md:w-8/12 lg:w-6/12 xl:w-5/12 2xl:w-4/12 ${details ? 'max-h-screen' : 'max-h-64 sm:max-h-96'} flex justify-center bg-gradient-to-r from-blue-500 to-indigo-600 border-2 backdrop-blur-xl rounded-lg shadow-lg hover:scale-105 transition-all duration-500 animate-fadeIn ease-in-out`}>
                        <div className={`snow absolute inset-0 ${weather.main.temp < 0 ? 'block' : 'hidden'}`}>
                            <div></div>
                        </div>
